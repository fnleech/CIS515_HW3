...
\medskip
(6)
Choose $u_i \in f(\s{A}), \; u_i = f(a_i), \; a_i \in \s{A}$ and $\lambda_i \in \reals, \; \sum_i \lambda_i = 1$. The affine combination of $u_i$ is $\sum_i \lambda_i u_i = f(\sum_i \lambda_i a_i)$. Because $\s{A}$ is affine subspace then $\sum_i \lambda_i a_i \in \s{A}$, which means $\sum_i \lambda_i u_i \in f(\s{A})$, then $f(\s{A})$ is affine subspace too. \\
